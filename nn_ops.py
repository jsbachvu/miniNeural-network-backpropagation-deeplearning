...
def initNetwork(nn_arch, act_func_name):
    ...
    for i in range(np.size(nn_arch)-1):
        ...
        #w = 0.01*np.random.randint(1,80,[nn_arch[i+1],nn_arch[i]])
        w = np.random.normal(size=(nn_arch[i+1],nn_arch[i]))
        W.append(w)
...
def forward(act_func,W,B,X):
    ...
    for w,b in zip(W,B):
        wx = np.dot(w, Y[-1]) + b
        act, act_derive = act_func(wx);
        Y.append(act)
        Yp.append(act_derive)
...
def backward(error,W,Yp):
    ...
    for i in reversed(range(1,len(W))):
        #e = gradB[-1] * w * Yp[i]
        #gradB.append(e)
        e = np.multiply( np.dot(W[i].T, gradB[0]), Yp[i-1] )
        gradB.insert(0,e)
        
    #####################
    return gradB #reversed(gradB) # gradB(::-1)
...
def update(eta, batch_size, W, B, gradB, Y):
    ...
    for i in range(len(W)):
        W[i] = updateParams(W[i], np.dot(gradB[i], Y[i].T)/batch_size, eta)
        B[i] = updateParams(B[i], (np.sum(gradB[i], axis=1).reshape(-1,1)/batch_size), eta)
